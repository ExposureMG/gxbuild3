...
namespace {
    ...
    bool crypt_single_bl(std::vector<uint8_t>& data, HmacType hmac_type,
                         uint8_t cur_key[16], const uint8_t cpu_key[16], uint8_t cba_hdr[16]) {
        if (data.size() < 32) {
            return false;
        }
        ...
        ExCryptRc4(cur_key, 16, data.data() + 0x20, static_cast<uint32_t>(data.size() - 0x20));
        return true;
    }
                ...
                if ((bl.flags & 0x800) == 0x800) {
                    ...
                    if (bl_count[0x4342] > 1) {
                        ...
                        if (bl.build >= 1920 && bl.data.size() >= 0x40 &&
                            !std::all_of(bl.data.begin() + 0x20, bl.data.begin() + 0x40, [](uint8_t b) { return b == 0; })) {
...
std::optional<std::vector<uint8_t>> pack_and_encrypt_bootloaders(const flash_image_t& image,
    ...
    std::vector<std::vector<uint8_t>> raw_stages;
    if (image.cb_or_A) raw_stages.push_back(*image.cb_or_A);
    if (image.cb_X) raw_stages.push_back(*image.cb_X);
    if (image.cb_B) raw_stages.push_back(*image.cb_B);
    if (image.sc) raw_stages.push_back(*image.sc);
    if (image.cd) raw_stages.push_back(*image.cd);
    if (image.ce) raw_stages.push_back(*image.ce);

    if (raw_stages.empty()) {
        return std::nullopt;
    }

    std::vector<BootloaderBlock> bls;
    for (auto& stage_data : raw_stages) {
        if (stage_data.size() < 16) {
            continue;
        }
        BootloaderBlock bl{};
        bl.magic = (static_cast<uint16_t>(stage_data[0]) << 8) | stage_data[1];
        bl.build = (static_cast<uint16_t>(stage_data[2]) << 8) | stage_data[3];
        bl.flags = (static_cast<uint16_t>(stage_data[6]) << 8) | stage_data[7];
        bl.size = (static_cast<uint32_t>(stage_data[12]) << 24) |
                  (static_cast<uint32_t>(stage_data[13]) << 16) |
                  (static_cast<uint32_t>(stage_data[14]) << 8) |
                  static_cast<uint32_t>(stage_data[15]);
        bl.data = std::move(stage_data);

        size_t target_len = (bl.size >= 16) ? ((bl.size + 0x0F) & ~0x0F) : ((bl.data.size() + 0x0F) & ~0x0F);
        if (bl.data.size() > target_len && bl.size >= 16) {
            bl.data.resize(target_len);
        } else if (bl.data.size() < target_len) {
            bl.data.resize(target_len, 0);
        }
        bls.push_back(std::move(bl));
    }

    if (!crypt_bootloaders(bls, cpu_key)) {
        return std::nullopt;
    }

    std::vector<uint8_t> out;
    for (const auto& bl : bls) {
        out.insert(out.end(), bl.data.begin(), bl.data.end());
    }
