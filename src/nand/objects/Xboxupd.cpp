...
XboxupdParts split_xboxupd_raw(std::span<const uint8_t> xboxupd_bytes) {
    ...
    if (cf_size < kBootloaderHeaderSize || xboxupd_bytes.size() < cf_size) {
        Log::Error("Invalid CF size 0x{:X} in xboxupd (buffer size 0x{:X})", cf_size, xboxupd_bytes.size());
    ...
    if (cg_size < kBootloaderHeaderSize || xboxupd_bytes.size() < (cg_offset + cg_size)) {
        Log::Error("Invalid CG size 0x{:X} in xboxupd (buffer size 0x{:X})", cg_size, xboxupd_bytes.size());
    ...
    Log::Debug("Split xboxupd.bin into CF (0x{:X} bytes) and CG (0x{:X} bytes)", cf_size, cg_size);
    return parts;
}
